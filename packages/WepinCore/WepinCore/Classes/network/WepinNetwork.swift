...
//                }
    ...
    func isErrorResponse (response: Data) -> String? {
        if let json = try? JSONSerialization.jsonObject(with: response, options: []) as? [String: Any] {
                // statusCode 또는 status 키 확인
                if let statusCode = json["statusCode"] as? Int ?? json["status"] as? Int {
    ...
    private func checkError(data: Data, response: URLResponse?) throws {
        ...
        if (200...299).contains(statusCode) {
            ...
            if let errorMessage = self.isErrorResponse(response: data) {
                if (errorMessage.contains("Invalid platform") || errorMessage.contains("Invalid apiKey")) {
                    throw WepinError.invalidAppKey
                } else {
    ...
    // MARK: - API Methods
    public func getNetworkInformation() async throws -> NetworkInfoResponse {
        guard let url = URL(string: "https://delivery.wepin.io/sdk/provider/info.json") else { throw WepinError.invalidRequest }
        var request = URLRequest(url: url)
