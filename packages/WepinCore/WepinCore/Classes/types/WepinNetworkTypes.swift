...
public struct JsonRpcUrl: Codable {
    public let type: String
...
public struct NetworkInfo: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        rpcUrl = try container.decode([JsonRpcUrl].self, forKey: .rpcUrl)
        isTestnet = try container.decodeIfPresent(Bool.self, forKey: .isTestnet) ?? false
        
        // chainId 처리: Int나 String 모두 받아서 String으로 변환
        if let intChainId = try? container.decode(Int.self, forKey: .chainId) {
            chainId = String(intChainId)
        } else {
    ...
    enum CodingKeys: String, CodingKey {
        case family, id, name, chainId, ticker, rpcUrl, isTestnet
    }
}

public struct NetworkInfoResponse: Codable {
