...
public class WepinSessionManager {
        ...
        do {
            ...
            let loginStatus = try await WepinNetwork.shared.getLoginStatus(userId: userId)
                let userStatus = StorageDataType.UserStatus(
                    ...
                    pinRequired: loginStatus.pinRequired ?? (loginStatus.loginStatus == WepinLoginStatus.pinRequired.rawValue)
                )
    ...
    public func checkExistFirebaseLoginSession() async  -> Bool {
        do {
            let token = WepinStorage.shared.getStorage(key: "firebase:wepin", type: StorageDataType.FirebaseWepin.self)
            ...
            let response = try await WepinFirebaseNetwork.shared.getRefreshIdToken(GetRefreshIdTokenRequest(refreshToken: token!.refreshToken))
            
            let newToken = StorageDataType.FirebaseWepin(idToken: response.idToken, refreshToken: token!.refreshToken, provider: token!.provider)
